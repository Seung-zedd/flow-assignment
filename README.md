...
- **담당 역할:** 1인 기획·설계·개발·검증, **MoAI-ADK 기반 AI 에이전트 오케스트레이션**
...
이 저장소는 단순히 "AI로 코드를 만들었다"는 결과보다, **어떤 판단을 AI에 위임하고 어떤 판단은 사람이 직접 가져가야 하는지 경계를 설계한 과정**을 함께 남긴 포트폴리오입니다.

요구사항과 인수 기준은 SPEC으로 먼저 고정하고, 기계적 구현과 검증 가능한 작업은 에이전트에게 적극적으로 위임했습니다. 반대로 기술 스택 선택, 보안 정책의 허용·차단 기준, 시크릿 처리 방식처럼 서비스의 성격을 바꾸는 결정은 직접 검토하거나 승인 게이트를 두었습니다.

[`PROMPT_LOG.md`](./PROMPT_LOG.md)에는 2026-08-31 기준 **46개의 판단 마커**를 기록했습니다.

| 판단 유형 | 건수 | 의미 |
|---|---:|---|
| 🟢 채택 | 24 (52%) | 추가 개입 없이 AI 제안을 그대로 통과 |
| 🟡 수정 후 채택 | 17 (37%) | 검증 루프나 사용자 개입 후 수정해서 반영 |
| 🔴 폐기 | 3 (7%) | 환경 제약, 사고, 미해결 이슈 등으로 채택하지 않음 |
| ⏳ 미정 | 2 (4%) | 후속 판단으로 해소된 보류 항목 |

수정이 필요했던 17건 중 **11건은 AI 검증 루프가 스스로 잡아 고쳤고, 6건은 사용자가 직접 적발**했습니다. 즉, 모든 생각을 AI에 넘긴 것이 아니라 **반복 가능하고 검증 가능한 판단은 위임하고, 모델 배정·보안 절차·정책 경계처럼 위험도가 높은 결정에 인간 개입을 집중**했습니다.

대표적인 예시는 다음과 같습니다.

- 기존 경험을 재사용한다는 이유만으로 Spring Boot를 고르지 않고, 프로젝트 규모와 배포 제약을 다시 비교해 **SvelteKit + Vercel**로 방향 전환
- SPEC 초안이 놓친 `jpeg/jpg` 별칭, 과도한 MIME 불일치 거부, prefix sniffing 정책을 **AI 오케스트레이터의 재검토 루프에서 발견·보완**
- "저녁 먹는 동안 알아서 진행하되 판단이 필요한 직전에는 멈춘다"는 식으로 **자율 진행 범위와 인간 승인 게이트를 명시**
- 보안 훅 이관 과정에서는 ESM, pnpm, `.env.example` 예외 등 환경 차이를 반영해 **원본을 그대로 복사하지 않고 수정 후 채택**

> 목표는 AI에게 생각을 전부 맡기는 것이 아니라, **위임 가능한 판단의 범위를 넓히되 최종 책임이 필요한 지점은 사람이 계속 소유하는 개발 방식**을 실험하는 것이었습니다.
...
- **`PROMPT_LOG.md`** — **AI 에이전트 오케스트레이션 기록** (요구사항 분해, 프롬프트 타임라인, 사용한 스킬·에이전트, 제안의 채택/수정/폐기 근거)
...
| 파일 | 내용 |
|---|---|
| [`PROMPT_LOG.md`](./PROMPT_LOG.md) | **AI 에이전트 오케스트레이션 기록** — 요구사항 분해, 프롬프트 타임라인, 사용한 스킬·에이전트, 제안의 채택/수정/폐기 근거와 인간 개입 지점 |
